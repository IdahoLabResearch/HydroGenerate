...
if(op=='From File'):
    ...
    #date_range = (dates[-1] - dates[0]).days # Calculating total number of unique days
    #next_day = [dates [0] + dt.timedelta(days=i) for i in range(len(flow_info['Date/Time']))]
    # days = mdates.drange(dates[0],next_day,dt.timedelta(days=1))
    ...
    p1,= host.plot(dates,(effi_cal*100),'g-',label="Efficiency")
    p2,= par1.plot(dates,power,'b-',label="Power")
