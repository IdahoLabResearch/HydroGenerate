...
g = 9.81
rho = 1000
turb_cap = 0 #Turbine nameplate capacity in MW '''Narrows_1 12 MW''' '''IFP(Lower) 8.3 MW''' ''Mottville 1.6 MW''
diff_m = 30 #ft '''263 ft for Yuba River Dam''' '''19ft IFP (lower)''' Mottville 23ft
max_flow_val = 0 #Provide design flow in cfs #730cfs Narrows_1?
'''
1 ft = 0.3048m # Ft to m conversion
'''

flow_info = pd.read_excel(r'C:\Work\My_Code\Files\scaled_hydropower.xlsx')
...
if (op == 'From File'):
    ...
    max_flow_val = max(flow_info['Average (cfs)'])
...
if (op == 'From File'):
    ...
    flow_info['Power (MW)'],flow_info['Efficiency (%)'],flow_info['Average_clipped (cfs)']=[power,effi_cal,flow_range/0.028316846591999675]
    flow_info.to_csv('C:\Work\My_Code\Files\energy_task.csv'.format(site_no,begin_date,end_date),index=False)
